...
defmodule Hw.Ariel2 do
  ...
  # @doc """
  # 2. The `insertion_sort` function takes an unsorted list of numbers as input and returns a new list
  # with the same elements but in ascending order. The `insert` function defined in the previous
  # exercise should be used to write `insertion_sort`. The `sort` or any similar predefined function
  # should not be used.
  # """
  ...
  # when there are cero rotations
  defp do_rotate_left(lst, 0, _temp),
    do: lst

  # when there are positive rotations (left)
  ...
  # @doc """
  # !!!!6. The `deep_reverse` function takes a list as input. Returns a list with the same elements as
  # its input but in reverse order. If there are nested lists, these should also be reversed.
  # """

  # @doc """
  # 7. The `insert_anywhere` function takes two inputs: an object `x` and a list `lst`. Returns a new
  ...
  # @doc """
  # 11. The `encode_modified` function takes a list `lst` as input. It works the same as the previous
  ...
  # @doc """
  # 13. The `args_swap` function takes a two_argument function `f` as input and returns a new function
  # that behaves like `f` but with the order of its two arguments exchanged. In other words:
  # ((args_swap f) x y) ≡ (f y x)
  # """
  ...
  # return `true` if `a` should be considered equal to `b`, or `false` otherwise. The `linear_search`
  ...
  # Some things to note:
  # _ `f` must be a differentiable real function.
  # _ The greater the value of `n`, the better the approximation.
  # _ `x0` is an initial approximation, and it is recommended to be a value close to the solution.
